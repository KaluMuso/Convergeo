...
class TicketFixture:
    ...
    this canonical identity.  The PIN is derived from the staging service-role key
    and sealed through the real `seal_pin_storage()` path; no usable credential is
    committed here.
    """
...
def canonical_contract_document() -> dict[str, Any]:
    ...
    ticket scanner PIN is deterministically derived from the environment's
    service-role secret and sealed into `pin_hash`; it is not fixture identity,
    so it must not shift the version hash.
    """
