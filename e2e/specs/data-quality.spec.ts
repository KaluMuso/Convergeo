...
test.describe("data-quality · browse surfaces", () => {
  test("home and PLP pass data-quality scan", async ({ page }) => {
    ...
    // PLP may not have PDP-specific checks — filter to price/demo/wholesale only.
  ...
  test("seeded PDP passes data-quality scan", async ({ page }) => {
    ...
    const unavailable = await page
      ...
      .isVisible()
      .catch(() => false);
    // Allow missing image when PDP is honestly unavailable.
  ...
  test("vendor storefront link resolves", async ({ page }) => {
    ...
    // Vendor page should render grid, empty, unavailable, or heading — not a blank 404.
    const surface = page
