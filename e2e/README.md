...
An identity-aware preflight probe (`scripts/ci/e2e-staging-probe.mjs`) verifies
customer `/en/health`, rejects Vercel SSO interstitials as `BLOCKED_EXTERNAL`,
and optionally checks `buildId` against the release-candidate SHA. Playwright
