...
// Reuses the shared jsQR-backed decode util from the order-pickup scanner
// (dynamic import, already in the vendor bundle) instead of loading a second
// QR decoding library.
import { decodeQrFromImageData } from "../../../../scan/_lib/qr-decode";
...
export function CameraScanner({ disabled, onCodeDetected, onCameraDenied }: CameraScannerProps) {
        ...
        const scanFrame = async () => {
          ...
          if (width > 0 && height > 0) {
            ...
            if (context) {
              ...
              if (code && (code !== lastCodeRef.current || now - lastCodeAtRef.current > 2000)) {
  ...
  if (mode === "denied") {
    return (
      <div
        data-testid="event-scan-camera-denied"
        style={{
          borderRadius: "var(--r)",
      ...
      >
        <p style={{ margin: 0, fontWeight: 600 }}>{t("scan.eventCheckIn.camera.denied")}</p>
        <p
          style={{ margin: "var(--sp-2) 0 0", color: "var(--text-2)", fontSize: "var(--fs-small)" }}
        >
          {t("scan.eventCheckIn.camera.deniedBody")}
        </p>
      </div>
    );
  }
  ...
  return (
    <div data-testid="event-scan-camera-active">
      ...
      >
        {t("scan.eventCheckIn.camera.hint")}
      </p>
    </div>
  );
}
