...
/**
 * Protected test-event path. Requires header `X-Sentry-Test-Secret` matching
 * server-only `SENTRY_TEST_SECRET`. Disabled in production unless
...
export async function POST(request: Request): Promise<Response> {
  ...
  const eventId = Sentry.captureMessage("Vergeo5 observability test event (customer)", {
    level: "error",
    tags: {
