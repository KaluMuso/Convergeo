...
| `NEXT_PUBLIC_API_BASE_URL`                         | —                | ✓ (customer/vendor)             | —              | Browser API origin. Production: `https://api.vergeo5.com`. Preview/staging: `https://api.staging.vergeo5.com`. Blank or loopback in a deployed build **fails closed** (no `localhost:8000` fallback). Admin uses `NEXT_PUBLIC_VERGEO_API_URL`. |
| `NEXT_PUBLIC_GOOGLE_AUTH_ENABLED`                  | —                | ✓ (customer/vendor/admin)       | —              | Exact string `true` to show the Google button. Default off. Never enable until the Supabase Google provider is live.                                                                                                                           |
