...
- **Recommended fix (test-only, ~2 lines):** seed clock-relative (derive the report date from where postings land / freeze the clock) rather than a literal `2026-07-18`. Surface as a small Wave-2 pebble; **do not** change ledger `created_at` semantics for this.
...
- **Recommended fix:** add the three files to the `rls` job's pytest invocation — **after** F-1, else CI goes red off-date.
...
## Status
Money code **CODE_COMPLETE — verdict UPHELD** · 2 non-blocking follow-ups (F-1 test date, F-2 CI wiring) · S1–S6 live walk founder-gated.
