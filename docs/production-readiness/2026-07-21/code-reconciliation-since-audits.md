...
note re-baselines the *repo-closable* items against current master so no one
...
| **R-2** | **API-base fail-closed sweep (residual)** | `(shop)` is fail-closed, but ~27 sites in `apps/customer/app/[locale]/account/**`, `beta`, and **`lib/seo/sitemap-events.ts`** still inline `?? "http://localhost:8000"` (sitemap = public SEO leak if env unset). Vendor/admin: audit VEND-11/ADM-11 — confirm their `lib/api-base-url.ts` is used everywhere. | Cursor (mechanical sweep) |
...
| **R-5** | **RLS test-matrix rows (RC-07, hygiene)** | `event_categories`,`product_relations`,`service_reviews` policied but absent from `tests/rls/test_matrix.py` registry (+`embedding_jobs`/`reconciliation_reports` legacy). | Cursor (small) |
...
- **DEPLOY**: frontends behind master SHA; **migrations `0064`–`0066` not applied live** (FORCE RLS still off on ticket-tier tables live — the top *safety* gap); API digest unpinned.
