...
WAHA is permitted **solely** as an **inbound, 1:1, verified-vendor product-intake** channel that produces private intake/staging records. It is:
...
- **strictly inbound-only** (it sends no acknowledgement or other message);
...
5. **Intake content only.** Only messages carrying product content (text + images) are processed into a private intake/staging record. The number never accepts or acts on commands, links to pay, support requests, or anything resembling a customer interaction.
...
- Create only an RLS-protected private intake/staging record and private media references owned by that vendor. Before M18-P05, the lane must not create, modify, or publish `vendor_listings`.
...
- **Publish** a listing, **approve** KYC, **move money**, or **take a moderation action** — the LLM and the WAHA lane are never the approver. A private intake record reaches the normal listing flow only through M18-P05's guarded, audited, vendor-confirmed handoff.
- Send transactional or marketing templates (that is Lane 1's job and stays there).
- Send an acknowledgement or any other outbound message. A future acknowledgement, if justified, must be a separately designed official Cloud API/outbox capability subject to its existing consent/template controls.
...
WAHA posts inbound events to a dedicated backend route (design target: `POST /webhooks/waha-intake`). The **pinned protocol contract is WAHA `2026.5.1`**: only its documented session `message` webhook event and signed raw-body envelope are accepted. A future implementation pins a compatible WAHA release and regression fixtures to this contract; an algorithm, header, or event-shape change needs an ADR and test update. Controls are **fail-closed**:

- **WAHA HMAC signature (fail-closed).** Before parsing JSON, the backend verifies `X-Webhook-Hmac` against an HMAC-**SHA-512** of the exact raw request body using `WAHA_INTAKE_WEBHOOK_SECRET` and `hmac.compare_digest`. `X-Webhook-Hmac-Algorithm` must be present and exactly `sha512`; missing, changed, or invalid headers ⇒ `403`, nothing parsed. **Do not reuse** Meta's `X-Hub-Signature-256` / `verify_hub_signature` verifier.
- **Request identity and freshness.** Require WAHA's `X-Webhook-Request-Id` and `X-Webhook-Timestamp` (Unix milliseconds) for attempt audit and freshness validation. Reject a timestamp outside the small skew window (design target ±5 min) before using any payload field.
...
- **Replay protection.** After authentication and schema validation, dedupe the signed WAHA message ID with `(provider='waha', event_id=<waha message id>)` through the existing `webhook_events` UNIQUE `(provider, event_id)` constraint. A replayed or retried message is a no-op; the request ID remains audit evidence, not a replacement for message-level idempotency.
- **No WAHA database access.** The WAHA process/connector receives no Supabase credential, especially no service-role key. Only the existing server-side FastAPI trust boundary may call the future P01 private-intake service after all checks above; it cannot reach money, auth, moderation, or `vendor_listings` paths. Rate-limited like every mutating route (declared in `ratelimit_policies.py` when built).
...
- **Kill switch = flip to `false`.** Effect is immediate and deploy-free: the ingestion handler's first check is the flag (§4.1), so every subsequent event is ignored. No private intake record is created and WAHA sends nothing.
...
- **Stage 1 — Pilot (founder-approved).** Entry gates, all required:
  1. Dedicated intake number provisioned and paired only through the founder-controlled intake-device/session procedure; **NB-7 separation proven** — `WAHA_INTAKE_SENDER_E164` ≠ Cloud API sender ≠ any `waha.vergeo.company`/ZedApply sender — and recorded here.
...
- **Stage 2 — Production (founder sign-off).** Only after the pilot meets exit criteria: no ban/quality event on the intake number; no group/customer/payment/support message ever processed (audit-verified); private-intake quality acceptable; the incident/kill-switch drill (Part B R4–R5) rehearsed once. Founder records the go decision; widening the vendor allowlist is itself an audited config change.
...
- **Per event:** authenticated WAHA request ID, message ID, received-at, sender MSISDN (minimised per §12), matched `vendor_id` **or** `null`, message type, media object refs, LLM-suggested fields, and **disposition** — one of `intake_created`, `dropped_group`, `dropped_unverified`, `dropped_flag_off`, `rejected_auth`, `error`.
- **Flag/allowlist changes:** captured by the `feature_flags` / `platform_config` → `config_audit` trigger (actor, before/after).
- **Intake → listing:** M18-P05 alone invokes the normal guarded vendor-listing handoff and its **admin/audit** trail. The WAHA lane's record ends at `intake_created`; it never creates a listing or records an approval.
...
- **Consent (opt-in, recorded).** A vendor is enrolled in WAHA intake only after an **explicit, timestamped opt-in** (vendor-app toggle or signed vendor-agreement clause), stored and audited like `profiles.dpa_consent_at` / vendor-agreement consent in `notification-compliance.md`. Enrolment states which number they message and that content is used solely to create private intake/staging records. Opt-out removes them from the allowlist immediately; a `STOP`-style keyword on the intake number also disenrolls. No vendor is ever cold-messaged on this number.
- **Data minimisation.** Only what's needed to create a private intake/staging record. Sender MSISDN is matched then stored **minimised** (reference the vendor row, not a free-floating phone log). No customer PII ever transits this lane (it is vendor-only by construction).
- **Retention (aligned to `data-retention.md`).**
  - **Raw inbound message text/media** used for extraction: kept only until the private intake record is created and the vendor has reviewed it, then **purged** on a short window (design target ≤ 30 days), via an idempotent, service-role sweep + n8n tick — the same shape as the analytics-retention sweeper.
  - **Product images the vendor later chooses to hand off through M18-P05** become normal listing media under the standard media pipeline and its lifecycle (not "message data").
...
- [ ] Dedicated `+260` intake number acquired, **not** the Cloud API number and **not** the agency `waha.vergeo.company` number. Register/pair it only through the documented founder-controlled intake-device/session procedure; do not share or reuse that account. Record it as `WAHA_INTAKE_SENDER_E164` and **write the three-way "different number" confirmation into §10 Stage 1**.
...
4. Send a test product message **from a pilot vendor's verified number** → confirm: private intake record created, **no WAHA outbound message**, audit row `intake_created`, and **no** customer contact anywhere.
5. Send from a **non-verified** number → confirm `dropped_unverified`, no reply.
6. Add the test number to a WhatsApp **group** and message there → confirm `dropped_group`, no reply, no private intake record.
...
WAHA intake is fully reversible: set `waha_vendor_intake = false`, stop the WAHA container, revoke the session, delete the `WAHA_INTAKE_*` secrets, and (optionally) release the intake number. Private intake records already created remain vendor-scoped for the later M18-P05 review/handoff or retention deletion; no customer-facing surface ever depended on this lane, so removal is invisible to customers.
