...
> **Money-workflow safety (VD-P06):** the HTTP tick nodes in `release-job`,
> `order-jobs`, `payment-sweeper`, `reconciliation`, `tickets-issue`,
> `tickets-release` and `event-release` carry `retryOnFail` (3 tries, 5 s apart);
> idempotency lives in the API so retries are safe. On import, additionally set
> each of these workflows' **Settings → Error Workflow** to a founder-alert
> workflow (reuse the WhatsApp page pattern from `uptime-alert.json`) so a
> non-2xx money tick pages the founder — the error-workflow binding is an
> instance-level ID and cannot be pinned in these JSON exports.
...
| `uptime-alert.json`          | Webhook (UptimeRobot)     | WhatsApp Cloud API (direct)                          | Page founder on WhatsApp when a monitor trips (API-independent). Inbound webhook is shared-secret-guarded: configure the UptimeRobot URL as `…/webhook/uptime-alert?secret=<UPTIME_WEBHOOK_SECRET>` and set `UPTIME_WEBHOOK_SECRET` in the n8n env — requests without the matching secret are dropped | M16-P06 |
| `backup.json`                | Daily 02:00 Africa/Lusaka | `infra/scripts/db-dump.sh` (Execute Command on host) | Nightly logical DB dump → OCI Object Storage per `infra/n8n/backup-schedule.md` (retention 14d); non-zero exit retries once (5 min) then pages the founder on WhatsApp. Requires the n8n container to mount `infra/` and its `.env` (see backup-schedule.md)                                          | VD-P04  |
