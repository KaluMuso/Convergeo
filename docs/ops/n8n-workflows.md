...
- Internal tokens are per-concern: `INTERNAL_N8N_TOKEN` (M14 operational nudges),
  ...
  `INTERNAL_EVENT_REFUND_JOBS_TOKEN`). Set each on the API and mirror it in n8n `$env`.
...
| `daily-summary.json`              | Daily 06:05 UTC                                              | `POST /internal/sweepers/daily-summary`                              | Push daily GMV, order count, and new-user count to `N8N_WEBHOOK_URL` for downstream n8n automations                                                                                                                                                                     | sweepers          |
...
| `preferred-badge-recompute.json`  | Daily 00:00 UTC                                              | `POST /internal/n8n/preferred-badge/tick`                            | Recompute D9 preferred-seller badges (inactive until founder activates; ≥20 completed orders, ≥4.5★, dispute <2%, cancel <5%)                                                                                                                                           | D9 / TRUST-OPS    |
...
| `event-refund-jobs.json`          | Every 5m                                                     | `POST /internal/event-refund-jobs/tick`                              | Sweep organiser-cancellation refund jobs (`event_refund_jobs`) — executes `execute_refund` off the cancel HTTP path; fails the tick when `dead` or `manual_review` counters are non-zero (link `money-workflow-error-alert.json` before Wave B activation)              | Prompt G / VD-P06 |
