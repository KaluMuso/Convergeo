...
| `reconciliation.json`             | Every 30m + daily 05:00                                      | `POST /internal/reconciliation/poll-tick`             | Poll Lenco + emit daily reconciliation report (+ retry + error-page)                                   | M08-P07 / VD-P06 |
...
| `backup.json`                     | Daily 02:00 Africa/Lusaka (+ 04:00 watchdog; manual webhook) | SSH → `infra/scripts/db-dump.sh` (OCI Object Storage) | Independent nightly logical DB dump + missed-schedule watchdog + failure alerts; restore-drill webhook | VD-P04           |
