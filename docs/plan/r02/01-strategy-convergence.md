...
| 1.8 | +260 phone normalisation, multi-line            | **Partial**                              | The only MSISDN normaliser in the codebase is `app/services/intake/sessions.py:38` `^260[79][0-9]{8}$`, scoped to the **dark** D35 lane. `addresses.phone` is free text; `vendors.whatsapp_msisdn` (`0046`) is a single storefront number. No shared normaliser, no multi-line support | Bible p.78; no locked decision                                                              | ADR-R02-03; **not** in the 12         |
...
| 9.8 | Deploy traceability                 | **Partial — a real code gap**          | `/fingerprint` exists (`services/api/app/routers/health.py:33-47`) but defaults `git_sha` to the literal `"unknown"` when unset, and the 2026-07-23 live probe recorded exactly `GIT_SHA=unknown`. A deploy that cannot name its own commit cannot be verified or rolled back with confidence                                                  | **R02-P02**                  |
...
**R02-P02 — Deploy fingerprint and live data-truth probes**
_Why:_ RG-1 is partly UNKNOWN because the running API cannot name its own commit
(`health.py:41` → `"unknown"`, observed live 2026-07-23).
_Scope:_ stamp `GIT_SHA` + `API_IMAGE_TAG` at deploy; make `verify_live.sh` **fail** (not warn) on
`git_sha=unknown`; add read-only data-truth probes to the same matrix — migration ledger tip vs repo
tip, `vendor_locations > 0`, `search_documents` geo coverage, publicly-discoverable listing count.
_Owns:_ `infra/redeploy-api.sh` · `infra/staging/redeploy-api-staging.sh` · `scripts/ops/verify_live.sh` ·
`.github/workflows/deploy-production.yml` · `services/api/app/routers/health.py`.
_Deps:_ none.
...
_Scope:_ extend `scripts/ci/migration-replay.sh` (which already has the duplicate-prefix guard) with a
live-ledger-vs-repo-filename diff that fails on key-shape drift; write the verified ordered apply plan
for `0072`→`0079` with the post-apply assertions (ledger tip `0079`; `clips`, `clips_comments`,
`waha_vendor_intake` rows exist and read `false`).
_Owns:_ `scripts/ci/migration-replay.sh` · `docs/ops/supabase-workflow.md` ·
**new** `docs/plan/r02/02-migration-apply-plan.md`.
_Deps:_ P02 (reuses its ledger probe). **Does not apply anything** — the apply is the operator's act (§6).
...
`apps/vendor/app/[locale]/listings/**` · `packages/i18n/messages/en/vendor.json` (new keys only).
_Deps:_ P01.
...
**R02-P06 — Bemba/Nyanja parity, part 1: `marketing`, `orders`, `account`**
The three worst customer-facing namespaces (9% / 28% / 35% translated).
_Owns:_ `packages/i18n/messages/{bem,nya}/{marketing,orders,account}.json` **only**.
_Deps:_ none.

**R02-P07 — Bemba/Nyanja parity, part 2: `catalog`, `checkout`, `directory`, `search`, `auth`**
`checkout` is money-path copy and D27 forbids unreviewed machine translation there — this pebble
produces candidate strings and marks them **unreviewed**; it does not certify them.
_Owns:_ `packages/i18n/messages/{bem,nya}/{catalog,checkout,directory,search,auth}.json` **only**.
_Deps:_ none (file-disjoint from P06).

**R02-P08 — Locale parity gate and human-review ledger**
_Scope:_ extend `scripts/ci/i18n-lint.mjs` with a per-locale customer-critical assertion — key present
**and not byte-identical to English** for a declared key set (the 1120 identical Bemba keys are exactly
what a presence-only check misses); add a review ledger recording who reviewed which namespace on what
date, with `legal` explicitly recorded as **deliberately English**.
_Owns:_ `scripts/ci/i18n-lint.mjs` · `packages/i18n/src/phase1-critical.ts` ·
`.github/workflows/perf.yml` · **new** `docs/plan/i18n-review-ledger.md`.
_Deps:_ P06, P07.
...
delivery band for an intercity address. The only MSISDN normaliser in the codebase
(`services/intake/sessions.py:38`) is scoped to the dark D35 lane.
_Recommendation:_ **yes on the phone normaliser and on making GPS required for delivery** (pickup
addresses may stay GPS-less); **defer the full structured-address schema** — landmark+GPS is the right
Zambian primitive and D16 keeps delivery Lusaka-only for now.
