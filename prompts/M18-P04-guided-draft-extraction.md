...
- **The one permitted outbound (`D35` §5):** a **single** automated receipt acknowledgement to the **same** verified vendor, i18n-keyed, no marketing. **Follow-up questions are part of that same 1:1 operational lane.** Never a customer, never a group, never a template send through Lane 1's customer templates.
...
Conversation orchestration only: guide the direct-chat vendor to a complete draft, extract fields **rules-first**, optionally augment with a **schema-constrained** model that cannot act, label every field's provenance, and ask concise follow-ups.
...
- **Modify:** `packages/i18n/messages/en/notifications.json` (**add a nested `intake` section** for the ack + follow-up copy — grep how the notifications i18n dir is loaded by `webhooks_whatsapp.py::_notifications_i18n_dir` and match it exactly)
...
### `orchestrator.py` — the 1:1 conversation

- Determines what is missing/contradictory and asks **concise, one-at-a-time follow-ups** (i18n-keyed; EN + Bemba/Nyanja slots) over the intake lane. Contradictions (two different prices, quantity vs stock mode conflict) are **surfaced as questions, never silently resolved**.
...
- Sends the **single** receipt acknowledgement to the same verified vendor. **Rate-limit outbound per session** so a loop or a chatty vendor can never turn the lane into a message pump. Never messages anyone else, ever.
...
Backend only; all vendor-visible copy is i18n-keyed (zero hardcoded strings). **Security:** untrusted input is never an instruction; the model has no tools and cannot act; schema-invalid output is discarded; prohibited fence reused not re-implemented; money via `Decimal` → integer ngwee; outbound is 1:1, rate-limited, and single-ack; **nothing here approves or publishes.**
...
`test_intake_orchestrator.py`: **incomplete** ⇒ concise follow-up + `needs_details` · **complete** ⇒ `ready_for_vendor_review` · **contradictory price** ⇒ question, not silent pick · **duplicate submission** ⇒ flagged, not auto-merged · **single ack only** (no second ack on replay; outbound rate-limit holds) · **AI unavailable/killed** ⇒ rules-only, session still progresses · **orchestrator never sets `submitted`/`approved`**. Full `uv run pytest` + `ruff check` + `mypy`.
...
- [ ] Exactly **one** ack, 1:1, rate-limited, i18n-keyed; never a customer, never a group.
...
**TESTS:** paste both prompt-injection cases + schema-invalid-discard + price-ngwee + AI-unavailable-degrade output, and the full-pytest tail
